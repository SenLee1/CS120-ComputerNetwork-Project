	...
	for (auto actualOutputChannel = 0; actualOutputChannel < maxOutputChannels; ++actualOutputChannel)
	{
		...
		else
		{
			...
			case Receiving:
				...
				while (!dataBeginFlag && receivedData.size() >= PREAMBLE_LENGTH)
				{
					sum = 0;
					for (int i = 0; i < PREAMBLE_LENGTH; ++i)
					{
				...
				while (dataBeginFlag && receivedData.size())
				{
			...
			case AckSending:
				bufferToFill.buffer->clear();
				for (int i = 0; i < bufferSize; ++i, ++AckreadPointer)
				{
					if (AckreadPointer < ackWave.size())
					{
						bufferToFill.buffer->addSample(actualOutputChannel, i, ackWave[AckreadPointer]);
					}
					else
					{
						setAckReadPointer(0);
