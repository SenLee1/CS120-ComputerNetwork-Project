...
class AudioNetworkSystem : public AudioIODeviceCallback {
  public:
    AudioNetworkSystem()
        : senderState(SenderState::Idle), receiverState(ReceiverState::Idle),
          samplesPerBit(0), samplesPerHalfBit(0), currentSampleRate(0.0),
          sendBufferPosition(0), sendSampleBufferEnd(0), receiverBufferPos(0),
        ...
        std::cout << "Output file: " << outputFile.getFullPathName()
                  << std::endl;
        std::cout << "Waves Saved At: " << outputWavFile.getFullPathName()
                  << std::endl;
    ...
    void audioDeviceAboutToStart(AudioIODevice *device) override {
        ...
        if (fmod(currentSampleRate, PhyConfig::BIT_RATE) != 0.0) {
            std::cerr << "Error: SampleRate (" << currentSampleRate
                      << ") can not exactly divided by (" << PhyConfig::BIT_RATE
                      << ")" << std::endl;
    ...
    void startSendFile() {
        ...
        std::cout << "Start Sending: " << inputFile.getFullPathName()
                  << std::endl;
        ...
        std::cout << "Frame Construction Completed. Total byte: "
                  << frame.size() << std::endl;
        ...
        if (currentSampleRate > 0 && !sendSampleBuffer.empty()) {
            ...
            std::cerr << "Warning: Failed to Save WAV File (SampleRate="
                      << currentSampleRate
                      << ", BufferEmpty=" << sendSampleBuffer.empty() << ")"
                      << std::endl;
        }
    ...
    void startReceiving() {
        ...
        // 重置所有接收状态机变量
        receiverState =
            ReceiverState::Idle; // 将在 processReceivedSample 中切换到 Syncing
        receiverBufferPos = 0;
  ...
  private:
    ...
    void modulateFrame(const std::vector<uint8_t> &frame) {
        ...
        std::cout << "Modulate Completed, Total Sample: " << sendSampleBufferEnd
                  << std::endl;
    ...
    void processReceivedByte(uint8_t byte) {
        ...
        case ReceiverState::Syncing: {
            ...
            if (byte == FrameConfig::PREAMBLE_BYTE) {
                ...
                // 找到了 SOF! 准备读取长度
                std::cout << "\n[RX] SOF Detected，Reading Length..."
                          << std::endl;
        ...
        case ReceiverState::ReadingLength: {
            ...
            if (receiverByteCount == 0) {
                ...
                std::cout << "[RX] Reading Completed: " << expectedPayloadLength
                          << " bytes is detected" << std::endl;

                // 安全检查
                if (expectedPayloadLength == 0 ||
                    expectedPayloadLength > 10000) // 10k 字节
                {
                    std::cerr << "[RX] Error: Invalid Length "
        ...
        case ReceiverState::ReadingPayload: {
            ...
            if (receiverByteCount == expectedPayloadLength) {
                // 有效载荷读取完毕，准备读取校验和
                std::cout << "[RX] Receving Payload Completed，Start Reading "
                             "and Checking..."
                          << std::endl;
        ...
        case ReceiverState::ReadingChecksum: {
            ...
            if (byte == calculatedChecksum) {
                ...
                // 校验和失败
                std::cerr << "[RX] Check Sum Failed! Expectation: "
                          << (int)calculatedChecksum << ", 收到: " << (int)byte
                          << std::endl;
                completionFlag.store(3); // 3 = 接收失败
            }
...
int main(int argc, char *argv[]) {
    ...
    if (dev_info.sampleRate != 48000.0) {
        std::cout << "SampleRate is not 48000HZ. Attempting to Set it."
                  << std::endl;
        ...
        dev_manager.getAudioDeviceSetup(dev_info);
        if (dev_info.sampleRate != 48000.0) {
            std::cerr << "!! Warning: Fail to Set SampleRate As 48000 Hz."
                      << std::endl;
            std::cerr << "!! It may lead to errors. Please configure your "
                         "sound card manually."
                      << std::endl;
        }
    }
    ...
    std::cout << " send    - (NODE 1) send INPUT.bin (save file as .wav)"
              << std::endl;
    std::cout << " receive - (NODE 2) receive and save file as OUTPUT.bin"
              << std::endl;
    ...
    while (running) {
        ...
        if (input == "send") {
            ...
            // 在主线程中等待，直到接收完成
            std::cout << "Waiting For Signal... (Enter 'stop' to Stop)"
                      << std::endl;
            int flag = 0;
            ...
            // 处理结果
            if (flag == 2) {
                std::cout << "\nReceived Successfully! File saved at."
                          << std::endl;
            } else if (flag == 3) {
                std::cout
                    << "\nReceived Failed! CheckSum Error or Invalid Length"
                    << std::endl;
            } else if (flag == 9) {
