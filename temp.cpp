...
{
private:
    std::vector<float> recordedData;  // 录音数据
    std::vector<float> mp3Buffer;     // MP3音频数据
    std::vector<float> generatedWave; // 生成的声波数据

    // 二进制传输相关数据
    std::vector<float> carrierWave; // 载波波形(1bit)
    std::vector<float> zeroWave;    // 零载波波形(0bit)
    ...
    std::vector<float> sentData;    // 待发送的数据波形
    std::deque<float> receivedData; // 接收的数据
    ...
    std::atomic<TransportState> transportState{Ready};

    // 播放位置
    std::atomic<size_t> playPosition{0};     // 录音播放位置
    std::atomic<size_t> mp3PlayPosition{0};  // MP3播放位置
    std::atomic<size_t> wavePlayPosition{0}; // 生成波形播放位置
    std::atomic<int> sendReadPointer{0};     // 发送数据读取位置

    // 接收相关变量
    std::atomic<float> sum{0};
    std::atomic<float> maxSum{SUM_THRESHOLD};
    std::atomic<int> lowerTicks{0};
    std::atomic<int> bitsReceived{0};
    std::atomic<bool> begin{false};
    std::atomic<bool> preambleSuspected{false};
...
public:
    ...
    void generateCustomWave(double freq1, double freq2, double duration = 5.0, int sampleRate = 48000)
    {
        ...
        std::cout << "生成自定义声波: f(t) = sin(2π·" << freq1 << "t) + sin(2π·" << freq2 << "t)" << std::endl;
        ...
        std::cout << "自定义声波生成成功" << std::endl;
        std::cout << "正在播放自定义声波..." << std::endl;
    ...
    void openFileForTransmission(const String &filePath)
    {
        ...
        if (file.hasFileExtension(".txt"))
        {
            ...
            transportState = ToSend;
            std::cout << "文件已加载，准备发送" << std::endl;
        }
        else if (!file.hasFileExtension())
        {
            // 处理输出目录(接收模式)
            outputFile = File(file.getFullPathName() + "/OUTPUT.txt");
            transportState = ToReceive;
            std::cout << "准备接收，输出文件将保存至: " << outputFile.getFullPathName() << std::endl;
        }
    }

    void startSending()
    {
        if (transportState != ToSend)
            return;
    ...
    void stopReceiving()
    {
        if (transportState != Receiving)
            return;

        transportState = Ready;
        std::cout << "接收已停止，共接收 " << bitsReceived << " 位" << std::endl;
    }
    ...
    void stopAll()
    {
        ...
        std::cout << "最终样本数: " << recordedData.size() << std::endl;
    }
    ...
    void startPlayingRecording()
    {
        ...
        {
            std::cout << "错误：没有录音数据，请先录音" << std::endl;
            return;
        }
        isPlaying = true;
        isRecording = false;
                               ...
                               int numSamples) override
    {
        ...
        if (outputChannelData != nullptr && numOutputChannels > 0)
        {
            for (int i = 0; i < numSamples; i++)
            {
                ...
                // 发送二进制数据
                if (transportState == Sending && sendReadPointer < sentData.size())
                {
                    outputSample += sentData[sendReadPointer];
                    sendReadPointer++;
        ...
        // 3. 接收处理
        if (transportState == Receiving && inputChannelData != nullptr && numInputChannels > 0)
        {
            for (int i = 0; i < numSamples; i++)
            {
                receivedData.push_back(inputChannelData[0][i]);
            }
            ...
            while (receivedData.size() >= PREAMBLE_LENGTH && !begin)
            {
                ...
                {
                    begin = true;
                    std::cout << "检测到前导码，开始接收数据..." << std::endl;
                }
            ...
            while (begin && receivedData.size() >= BIT_WIDTH)
            {
                ...
                for (int i = 0; i < BIT_WIDTH; i++)
                {
                    sum += receivedData[i] * carrierWave[i];
                }

                // 写入文件
                if (outputFile.exists())
                {
                    FileOutputStream oStream(outputFile, File::append);
                    oStream.writeText(sum < 0 ? "0" : "1", false, false, nullptr);
                }
                ...
                if (bitsReceived >= BIT_NUM)
                {
                    transportState = Ready;
                    begin = false;
                    std::cout << "数据接收完成，共接收 " << BIT_NUM << " 位" << std::endl;
                    break;
                }
            }
        }
    ...
    void checkPlaybackCompletion()
    {
        ...
        {
            isPlaying = false;
            std::cout << "录音播放结束" << std::endl;
        }
        if (isPlayingMP3 && mp3PlayPosition >= mp3Buffer.size())
        {
            isPlayingMP3 = false;
            std::cout << "MP3播放结束" << std::endl;
        }
        if (isPlayingGeneratedWave && wavePlayPosition >= generatedWave.size())
        {
            isPlayingGeneratedWave = false;
            std::cout << "生成波形播放结束" << std::endl;
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "3 <f1> <f2> - 生成两个指定频率的声波, 例如: 3 1000 2000" << std::endl;
    std::cout << "l <路径> - 加载MP3文件或传输用的TXT文件/目录" << std::endl;
    ...
    while (running)
    {
        ...
        { // 自定义频率声波
            ...
            {
                std::cout << "错误: 请提供两个频率参数，例如: 3 1000 2000" << std::endl;
            }
        }
        else if (command == "l")
        { // 加载文件
